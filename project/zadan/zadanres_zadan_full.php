...
<style>
	...
	table.reserve, table.reserve input.get_from_wh_req_input, table.reserve input.get_from_wh_req_comment_input
	{
		width : 100% !IMPORTANT;
	}
...
$( function()
{
	...
	$('.multiselect_checkbox').unbind('click').bind('click', multiselectCheckboxClick );		
});

function multiselectCheckboxClick()
{
	$( '.empty_emp_check' ).prop('checked', $( this ).prop('checked') );
}
...
{
	let id = $( this ).data('id');
	let id_zakdet = $( this ).data('zakdet');
	let wh_id = $( this ).data('wh-id');
	let warehouse_name = $( this ).data('warehouse-name');
	let cell_name = $( this ).data('cell-name');
	let tier_name = $( this ).data('tier-name');  
...
create : function()
{
	OpenWarehouseDialog( id, id_zakdet, pattern )        	
},
open : function()
{
	OpenWarehouseDialog( id, id_zakdet, pattern );
...
function OpenWarehouseDialog( id_zadan, id_zakdet, pattern )
{
...
if (isset($_GET['delresid'])) {
	...
	if( $used_ignore )
	{
		...
		// Быстрый костыль на промотку скролла на следующий после удаленного элемент (ресурс)
		...
		while($otdel = mysql_fetch_array($xxx)) 
		{
		   ...
		   ///////////////////////////////////////////
			...
			while ($res = mysql_fetch_array($xxx)) 
			{
				...
				if (in_array($res["ID_resurs"],$resurs_IDs) && $smena != 0 && $smena != "0") 
				{

				$query = "SELECT * FROM ".$db_prefix."db_zadanres where (DATE='".$pdate."') and (ID_resurs='".$res["ID_resurs"]."')";
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	   ...
	   // Ресурс
		...
		// var_dump($resource_status);
			...
			if( $task_count )
				...
				echo "<td class='Field AC' rowspan='2'><input type='checkbox' class='empty_emp_check' data-id='".$item["ID"]."'></td>";
	   ...
	   // Строка
		//echo "<tr id='row1_".$item["ID"]."' onmouseover=\"HLID(".$item["ID"].");\" onmouseout=\"DHLID(".$item["ID"].");\">";
			...
			$result = dbquery("SELECT 
					opitems.ORD AS ORD,
					opitems.STATE AS STATE,
					opitems.NUM_ZAK AS NUM_ZAK,
					opitems.NUM_ZADEL AS NUM_ZADEL,
					opitems.BRAK AS BRAK,
					opitems.NORM_ZAK AS NORM_ZAK,
					opitems.ID AS ID,
					opitems.NORM_FACT AS NORM_FACT,
					opitems.NORM AS NORM,
					coop.count AS count,
					coop.norm_hours AS norm_hours
					FROM ".$db_prefix."db_operitems AS opitems
					LEFT JOIN okb_db_operations_with_coop_dep AS coop ON coop.oper_id = opitems.ID
					WHERE 
					(opitems.ID = '".$item["ID_operitems"]."')");
	...
	$coop_count = $oper['count'] ? $oper['count'] : 0 ;
	$coop_norm_hours = $oper['norm_hours'] ? $oper['norm_hours'] : 0 ;
	...
	$draw_name = $izd["OBOZ"];
	if( strlen( $draw_name ) )
		$draw_where = " OR ( inv.draw_name LIKE '%$draw_name%')";

	$dse_pattern = $draw_name;	

	$query = "SELECT 
	detitem.ID AS ID, 
	detitem.ID_sklades_yarus AS ID_sklades_yarus, 
	...
	LEFT JOIN `okb_db_zadan` AS zadan ON zadan.ID = inv.id_zadan
	WHERE 
	(
	zadan.ID_zak=$id_zak
	AND
	zadan.ID_zakdet=$id_zakdet
	)
	$draw_where
	";

	// if( $id_zakdet == 591685 ) echo $query ;
	...
	if ( mysql_num_rows( $result ) ) 
	{
		...
		$link = "<span style='float:right'><a href='#'><img title='ДСЕ хранится на складе' class='warehouse' 
			data-id='".( $item["ID"] )."' 
			data-wh-id='$wh_id' data-warehouse-name='$warehouse_name' 
			data-cell-name='$cell_name' 
			data-tier-name='$tier_name' 
			data-pattern='$dse_pattern' 
			data-zak='$id_zak'
			data-zakdet='$id_zakdet'
			src='/style/packages.png' style='width:50%'/></a></span>";
	}
	...
	<span style='float:right'><img class='semifin_invoice' src='/style/report.png' style='width:70%;height:90%' title='Создать накладную на полуфабрикаты' /></span>
	$link
	...
	$oper_norm = $oper["NORM"]/(60);
	
	$oper_norm -=  - $coop_norm_hours ;
	$ost -= $coop_count;

	echo "<td class='Field'><center><b>". round(( $oper_norm * $ost ), 2) ." ($ost)</b><br>{$oper["NORM_ZAK"]} ($rcount)<br>".round($oper_norm, 2)."</center></td>";
...
// shindax 26.12.2017		
	if( $active )
	{
		...
		<select name='change_resource_select_date' style='text-align:center' size='5'></select>
		<select name='change_resource_select_date_resource' style='display:none;text-align:center;width:216px;margin-top:-78px' size='5'></select>
...
// *************************************************************************************************************      
	   ...
	   // ШАПКА ТАБЛИЦЫ ///////////////////////////////////////////////////////////////
			...
			echo "<td colspan='2' rowspan='2' width='50'>multiselect<br><input type='checkbox' class='multiselect_checkbox' /></td>\n";
			echo "</tr>\n";
