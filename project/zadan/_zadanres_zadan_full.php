...
echo "
<div id='warehouse_dialog' class='hidden' title='Выдать со склада'>
</div>";
...
function OpenZadanID( $item , $active = 1 ) 
    ...
    $link
	   ...
	   // Факт
		...
		if ($modering) 
			$calculator="<a href='javascript:void(0);' onClick=\"SetNewValue('".$from_id."','".$to_id."',".$nxx.",'".$churl."')\" title='Пересчитать Н/Ч'><img src='project/img/calc.png' alt='Пересчитать Н/Ч' class='recalc'></a>";
...
// shindax 13.06.2019
		...
		echo "<td class='Field nec' data-src='{$oper["STATE"]}'>
					$nec_select					
				</td>";
// shindax
	   ...
	   // Действие
		...
		<img alt='Готово' src='uses/ok_dis.png' class='hidden dis_img' data-id='{$item["ID"]}'>
		...
		else 
		{
			...
			if ((db_adcheck("db_zadan")) && ($oper["STATE"]=="0")) 

				echo " <a style='cursor:pointer;' onclick='reload_page(); vote7(this,".$item["ID"].",".$item["ID_operitems"].", ".$item['ID_resurs'].");'><img alt='Возобновить' src='uses/restore.png'></a>";
		}
		echo "</td>";
...
function vote6(obj, zadan_id, operit_id, id_res)
{
	...
	let zadan_arr = arr_zadan_id.split('|')
	...
	// let cause = $('select.noncomplete_execution_causes_select[data-id=' + arr_zadan_id + ']').eq(1).find('option:selected').val()

    });

	var req = getXmlHttp();
	req.open('GET', 'zadanres_okzad.php?id='+arr_zadan_id+'&operitems='+arr_operit_id+'&cause='+cause+'&user_id='+user_id);
	req.send(null);
}
