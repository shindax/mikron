...
$result5 = dbquery("SELECT COUNT(*) FROM ".$db_prefix."db_itrzadan where ((( ID_users2='".$name2['ID']."' ) OR ( ID_users2 LIKE '%".$name2['ID']."|%' ) ) and (STATUS='Новое')) ");
$name5 = mysql_fetch_row($result5);
$total5 = $name5[0];

$sdh = 0;
for ($sdg = 0; $sdg < $total5; $sdg++){
  $result6 = dbquery("SELECT * FROM ".$db_prefix."db_itrzadan where ( ( (ID_users2='".$name2['ID']."') OR ( ID_users2 LIKE '%".$name2['ID']."|%' ) ) and (STATUS='Новое') and (ID>'".$sdh."')) ");
  $name6 = mysql_fetch_array($result6);
  $total6 = $name6['ID_edo'];
  $total6_1 = $name6['ID'];
  $sdh = $total6_1;

  dbquery("UPDATE ".$db_prefix."db_itrzadan SET STATUS='Просмотрено' where ( ( ( ID_users2='".$name2['ID']."') OR ( ID_users2 LIKE '%".$name2['ID']."|%' ) ) and (STATUS='Новое') and (ID<'".($sdh+1)."')) ");
