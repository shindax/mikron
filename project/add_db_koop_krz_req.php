...
$result = dbquery("SELECT * FROM ".$db_prefix."db_koop_krz_req where (ID='".$insert_id."') ");
if ($row = mysql_fetch_array($result)) {
	...
	$res = dbquery("SELECT ID, NAME FROM ".$db_prefix."db_koop_krz_req where (CDATE > '".$ds."') and (CDATE < '".$de."') and (ID < '".$insert_id."') order by ID desc");
	...
	dbquery("Update ".$db_prefix."db_koop_krz_req Set NAME:='".$name."' where (ID='".$insert_id."')");
