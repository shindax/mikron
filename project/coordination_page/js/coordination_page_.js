...
let rows = $("tr[data-row]")

function check_disable_by_time()
{
  let time = new Date;
  ...
  {
    let input = $( item ).find('input.datepicker,input.agreed_flag,input.checkbox_input')
  ...
  $.each( rows , function( key, item )
  {
    ...
    let fill_period = $( item ).data('fill_period').split(",")
    
    let blk_time_from = fill_period[ 0 ].split(":")
    let blk_time_to = fill_period[ 1 ].split(":")

    let blk_hour_from = + blk_time_from[ 0 ]
    let blk_min_from = + blk_time_from[ 1 ]
    let blk_hour_to = + blk_time_to[ 0 ]
    let blk_min_to = + blk_time_to[ 1 ]

    let time = hour * 60 * 60 + min * 60 + sec
    blk_time_from = blk_hour_from * 60 * 60 + blk_min_from * 60
    blk_time_to = blk_hour_to * 60 * 60 + blk_min_to * 60

    if( row == 6 )
      cons(  blk_time_to + ' >= ' + time + ' >= ' + blk_time_from )

    // if( time >= blk_time_from && time <= blk_time_to )
    //   $( input ).removeClass('blocked-by-time').prop( 'disabled', false ).attr( 'title', '' ) 
    //   else
    //     $( input ).addClass('blocked-by-time').prop( 'disabled', true ).attr( 'title', '\u0411\u043B\u043E\u043A\u0438\u0440\u043E\u0432\u043A\u0430 \u043F\u043E \u0432\u0440\u0435\u043C\u0435\u043D\u0438' ) // "Блокировка по времени"
    // });
 }
}
  // check_disable_by_time()
  // setInterval( check_disable_by_time , 1000);
