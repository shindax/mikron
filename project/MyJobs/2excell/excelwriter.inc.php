...
EOH;
		...
		*/
		...
		function writeLine($line_arr, $class='')
		{
			if($this->state!="OPENED")
			{
				$this->error="Error : Please open the file.";
      ...
      if( $class == '' )
        $class = 'xl24';
			...
			foreach($line_arr as $col)
				...
				if (is_numeric($col))
					fwrite($this->fp,"<td x:num class=$class width=64 >".$col."</td>");
				else 
					fwrite($this->fp,"<td class=$class width=64 >".$col."</td>");
