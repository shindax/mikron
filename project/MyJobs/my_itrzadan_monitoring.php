...
foreach($itr_arr_7 as $keey_1 => $vaal_1)
{
  ...
  if( $prev_order_name != $order_name )
  {
    ...
    $key = $keey_1 ;
    $query = "
              SELECT prj.ID, prj.perc_of_execution, prj.STATUS, crt.name CR_NAME, exec.name EX_NAME, chk.name CH_NAME 
    ...
    echo "<tr class='project_head' data-opened='0' data-proj-id='$key'>
          <td class='Field AL project_title' data-proj-id='$key' colspan='4'>
          <img data-path='$EDIT_PROJECT_PAGE_ID' id='$id' class='edit_project_img' src='/uses/project.png'>
          $order_name&nbsp;<span>&#9658;</span></td>
...
//	echo "<tr data-id='2'>
  ...
  if( $prev_order_name != $order_name )
  {
    ...
    $query_ord = "
              SELECT zad.STATUS, zadcrt.name CR_NAME, zadexec.name EX_NAME, zadchk.name CH_NAME 
              FROM ".$db_prefix."db_itrzadan zad 
              INNER JOIN ".$db_prefix."db_resurs zadcrt ON zadcrt.ID = zad.ID_users 
              INNER JOIN ".$db_prefix."db_resurs zadexec ON zadexec.ID = zad.ID_users2 
              INNER JOIN ".$db_prefix."db_resurs zadchk ON zadchk.ID = zad.ID_users3 
              where zad.ID='$data_ord '";

    $result_ord = dbquery( $query_ord );
    $row_ord = mysql_fetch_assoc( $result_ord );

    $creator_ord = $row_ord['CR_NAME'];
    $executor_ord = $row_ord['EX_NAME'];
    $checker_ord = $row_ord['CH_NAME'];
    $state_ord = $row_ord['STATUS'];    

	echo "<tr class='ord_row hidden' data-proj='$key' data-ord='$data_ord'>
	<td class='Field' width='65px'><a href='index.php?do=show&formid=122&id=".$itr_arr_0[$keey_1]."'><img src='uses/view.gif' alt='Просмотр'></a>".$itr_arr_0[$keey_1]."</td>
	<td name='itrdate' class='Field' width='105px'>".IntToDate($itr_arr_1[$keey_1])."</td>
	<td name='factdate' class='Field' width='105px'>".$date_plan."</td>
	<td class='Field' width='115px'>".$itr_arr_8[$keey_1]." ".$itr_arr_7[$keey_1]."</td>
	<td class='Field' width='290' style='padding:6px;'>".$itr_arr_2[$keey_1]."</td>
	<td class='Field' width='150px'>$creator_ord</td>
	<td class='Field' width='150px'>$executor_ord</td>
	<td class='Field' width='150px'>$checker_ord</td>
	<td name='status' class='Field' width='85px'>".$itr_arr_6[$keey_1]."</td>";
	$ind_row_txt = "";
	if ($ind_row >= 5) 
        { 
