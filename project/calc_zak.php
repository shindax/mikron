...
function CalculateZakaz($id)
{
   ...
   /////////////////////////////////////////////
	...
	//Çàêàç
	$zak = dbquery("SELECT DSE_COUNT FROM ".$db_prefix."db_zak where (ID='".$id."')");
	$zak = mysql_fetch_assoc($zak);

	//First DSE
	$det = dbquery("SELECT ID FROM ".$db_prefix."db_zakdet where  (PID='0') and (ID_zak='".$id."')");
	...
	while($xdet = mysql_fetch_assoc($detres)) {
		...
		$operres = dbquery("SELECT ID, ID_zak, NORM_ZAK, (NORM + NORM_2) as NORM, NORM_FACT, FACT, BRAK, CHANCEL, NUM_ZAK, NUM_ZADEL FROM ".$db_prefix."db_operitems where (ID_zakdet='".$xdet["ID"]."') order by ID");
		while($xoper = mysql_fetch_assoc($operres)) {
			...
			while($zad = mysql_fetch_assoc($zadres)) {
				$fact += $zad["FACT"]*1;
			...
			if ($xoper["BRAK"]*1==0) {		//åñëè íå èñïðàâëåíèå áðàêà
			   if ($xoper["CHANCEL"]*1==0) {	//åñëè íå îòìåíà
				$normzak = number_format((($xdet["RCOUNT"]*1-$xoper["NUM_ZADEL"]*1)*($xoper["NORM"]/60)), 2, '.', '');
			   ...
			   if ($xoper["CHANCEL"]*1==0) {	//åñëè íå îòìåíà
				$normzak = number_format(($xoper["NUM_ZAK"]*($xoper["NORM"]/60)), 2, '.', '');
				if ($xoper["NORM_ZAK"]*1!==$normzak*1) dbquery("Update ".$db_prefix."db_operitems Set NORM_ZAK:='".$normzak."' where (ID='".$xoper["ID"]."')");
			   } else {				//åñëè îòìåíà: Í/× ïëàí = Í/× ôàêò
				$normzak = $norm_fact;
				if ($xoper["NORM_ZAK"]*1!==$normzak*1) dbquery("Update ".$db_prefix."db_operitems Set NORM_ZAK:='".$normzak."' where (ID='".$xoper["ID"]."')");
...
function CalculateOperitem($id) {
   ...
   /////////////////////////////////////////////
	...
	//operitem
	$operitem = dbquery("SELECT *, (NORM + NORM_2) as NORM FROM ".$db_prefix."db_operitems where (ID='".$id."')");
	$operitem = mysql_fetch_assoc($operitem);
	...
	if ($operitem["BRAK"]*1==0) {		//åñëè íå èñïðàâëåíèå áðàêà
	   if ($operitem["CHANCEL"]*1==0) {	//åñëè íå îòìåíà
		$normzak = number_format((($zakdet["RCOUNT"]*1-$operitem["NUM_ZADEL"]*1)*($operitem["NORM"]/60)), 2, '.', '');
	   ...
	   if ($operitem["CHANCEL"]*1==0) {	//åñëè íå îòìåíà
		$normzak = number_format(($operitem["NUM_ZAK"]*($operitem["NORM"]/60)), 2, '.', '');
		if ($operitem["NORM_ZAK"]*1!==$normzak*1) dbquery("Update ".$db_prefix."db_operitems Set NORM_ZAK:='".$normzak."' where (ID='".$id."')");
	   } else {				//åñëè îòìåíà: Í/× ïëàí = Í/× ôàêò
		$normzak = $norm_fact;
		if ($operitem["NORM_ZAK"]*1!==$normzak*1) dbquery("Update ".$db_prefix."db_operitems Set NORM_ZAK:='".$normzak."' where (ID='".$id."')");
	...
	$summ_n = 0;
	$summ_nv = 0;
	$result = dbquery("SELECT NORM_FACT,NORM_ZAK FROM ".$db_prefix."db_operitems where (ID_zakdet='".$zakdet["ID"]."')");
	...
	$zsumm_n = 0;
	$zsumm_nv = 0;
	$result = dbquery("SELECT NORM_FACT,NORM_ZAK FROM ".$db_prefix."db_operitems where (ID_zak='".$zakdet["ID_zak"]."')");
	while($item = mysql_fetch_assoc($result)) {
	...
	dbquery("Update ".$db_prefix."db_zak Set SUMM_N:='".$zsumm_n."',SUMM_NO:='".$zsumm_no."',SUMM_NV:='".$zsumm_nv."',SUMM_V:='".$zsumm_v."' where (ID='".$zakdet["ID_zak"]."')");
}
