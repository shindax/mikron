...
// для сравнения ИД того кто открыл мониторинг по таблице db_itrzadan
//$res1 = dbquery("SELECT * FROM ".$db_prefix."db_resurs where (ID_users='".$user['ID']."') ");
if (($user['ID']==3) or ($user['ID']==4) or ($user['ID']==16) or ($user['ID']==1) or ($user['ID']==91) or ($user['ID']==13)) { $wher_us = 4;}else{ $wher_us=$user['ID'];}
...
// ищим отдел где вышеуказанный ресурс является боссом без совместительства
...
// выводим инфу по полувшемуся дереву
//echo $child_otdels_1;
//echo $child_otdels_2;
//echo $child_otdels_3;
//echo $child_otdels_4;
...
$arch = $_GET['arch'];
if ($arch) { $itr_wher="and ((STATUS='Аннулировано') or (STATUS='Завершено'))";}else{ $itr_wher="and (STATUS!='Аннулировано') and (STATUS!='Завершено')";}
...
if ($_GET['spec_view']==2){
	...
	$href_spec_v = "<a href='index.php?do=show&formid=134".$p_url_1.$p_url_2."'>Открыть мониторинг заданий (заказы) в стандартном виде</a>";
}
if (!$_GET['spec_view']){
	$p_url_1 = "";
	$p_url_2 = "";
	if ($_GET["sort"]) $p_url_1 = "&sort=".$_GET["sort"];
	if ($_GET["arch"]) $p_url_2 = "&arch=".$_GET["arch"];
	$href_spec_v = "<a href='index.php?do=show&formid=134&spec_view=2".$p_url_1.$p_url_2."'>Открыть мониторинг заданий (заказы) по службам</a>";
}
...
function check_filtr_1(obj){
	...
	if(getUrlVars()['p1']){
		...
		if(obj.innerHTML.substr(0,3)=='(v)'){
			...
			history.replaceState(0, 'New page title', 'index.php?do=show&formid=134'+arch+spec_view+'&sort='+sort+'&p1='+filt_1);
			if (getUrlVars()['p1'].length<1){ history.replaceState(0, 'New page title', 'index.php?do=show&formid=134'+arch+spec_view+'&sort='+sort);}
		}else{
			obj.innerHTML = \"(v) \"+obj.innerHTML;
			history.replaceState(0, 'New page title', 'index.php?do=show&formid=134'+arch+spec_view+'&sort='+sort+'&p1='+obj.value+'|'+filt_1);
		}
	}else{
		...
		obj.innerHTML = \"(v) \"+obj.innerHTML;
		history.replaceState(0, 'New page title', 'index.php?do=show&formid=134'+arch+spec_view+'&sort='+sort+'&p1='+obj.value+'|');}
}
...
function check_filtr_2(obj){
	...
	if(getUrlVars()['p2']){
		...
		if(obj.innerHTML.substr(0,3)=='(v)'){
			obj.innerHTML = obj.innerHTML.substr(4);
			history.replaceState(0, 'New page title', 'index.php?do=show&formid=134'+arch+spec_view+'&sort='+sort+'&p2='+filt_2);
			if (getUrlVars()['p2'].length<1){ history.replaceState(0, 'New page title', 'index.php?do=show&formid=134'+arch+spec_view+'&sort='+sort);}
		}else{
			obj.innerHTML = \"(v) \"+obj.innerHTML;
			history.replaceState(0, 'New page title', 'index.php?do=show&formid=134'+arch+spec_view+'&sort='+sort+'&p2='+obj.value+'|'+filt_2);
		}
	}else{
		var pick_cur = document.getElementsByName('opt_filtr').length;
		for (var pick_for=0; pick_for<pick_cur;pick_for++){
		...
		obj.innerHTML = \"(v) \"+obj.innerHTML;
		history.replaceState(0, 'New page title', 'index.php?do=show&formid=134'+arch+spec_view+'&sort='+sort+'&p2='+obj.value+'|');}
}
...
function check_filtr_3(obj){
	...
	if(getUrlVars()['p3']){
		...
		if(obj.innerHTML.substr(0,3)=='(v)'){
			obj.innerHTML = obj.innerHTML.substr(4);
			history.replaceState(0, 'New page title', 'index.php?do=show&formid=134'+arch+spec_view+'&sort='+sort+'&p3='+filt_3);
			if (getUrlVars()['p3'].length<1){ history.replaceState(0, 'New page title', 'index.php?do=show&formid=134'+arch+spec_view+'&sort='+sort);}
		}else{
			obj.innerHTML = \"(v) \"+obj.innerHTML;
			history.replaceState(0, 'New page title', 'index.php?do=show&formid=134'+arch+spec_view+'&sort='+sort+'&p3='+obj.value+'|'+filt_3);
		}
	}else{
		var pick_cur = document.getElementsByName('opt_filtr').length;
		for (var pick_for=0; pick_for<pick_cur;pick_for++){
		...
		obj.innerHTML = \"(v) \"+obj.innerHTML;
		history.replaceState(0, 'New page title', 'index.php?do=show&formid=134'+arch+spec_view+'&sort='+sort+'&p3='+obj.value+'|');}
}
...
function check_filtr_clear(obj){
	...
	history.replaceState(0, 'New page title', 'index.php?do=show&formid=134'+arch+spec_view+'&sort='+sort);
}
...
//localhost/index.php?do=show&formid=134&spec_view=2&sort=5
$pred_otdel = "";
...
echo "<tr class='first'><td colspan='9'>Итого заданий<b>&nbsp;&nbsp;&nbsp;&nbsp;(".$itr_coun.")</b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
