...
function getCommentPriceOptions( $state )
{
    ...
    $query = "SELECT * FROM `okb_db_coop_pricing` WHERE 1 ORDER BY descr";
...
function getCommentStateOptions( $state )
{
    ...
    $query = "SELECT * FROM `okb_db_coop_task_state` WHERE 1 ORDER BY descr";
...
function getRequisitionTasks( $req_id )
{
    ...
    $query = "
                ...
                FROM okb_db_coop_tasks 
                LEFT JOIN okb_db_clients AS clients ON clients.ID = okb_db_coop_tasks.cagent_id
                WHERE  
                coop_req_id = $req_id 
			";
...
function getPageCaption( $req_id , $user_id )
{
	...
	$str = "<div class='row'><div class='col'><h2>".conv("Проработка и контрактация по заявке ")."#".$name.conv(" от ").$date."</h2></div></div>";
