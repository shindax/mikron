...
/* shindax	
// Костыль для устранения лишних "восьмёрок", если человек работал в отпуске
// Закомментировал исходный код
...
*/
  ...
  $sql = "SELECT * FROM ".$db_prefix."db_tabel where (DATE>'".$p_3_1."') and (DATE<'".$p_3_2."') order by DATE" ;
   ...
   /* if( ( $tid == 1 ) && ( $cur_fact  == $cur_plan ) && ( $cur_fact  != 0 ) && $weekday != 0 && $weekday != 6 )
        continue ;
*/
		$res_ids[] = $tab["ID_resurs"];
...
//				if (even_week($j+1,$DI_MM,$DI_YY)) {}
		...
		echo "<tr></tr><tr>
		<td width='40px'>
		<td colspan='9' width='600px' style='font-size:13pt; text-align:left;'>Начальник отдела кадров</td>
		<td colspan='12' width='300px'>_________________________________________</td>
		<td colspan='7' width='350px' style='font-size:13pt; text-align:left;'>".$xr3['NAME']."</td>
		</tr>";
		}
