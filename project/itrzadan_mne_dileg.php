...
echo"<h2>Делегирование заданий</h2><br>Для поиска используйте (F3)<br>";
...
$coun_ind = 0;
$res_2 = dbquery("SELECT * FROM okb_db_itrzadan where (ID_users2='".$nam_1['ID']."') and (STATUS!='Завершено') and (STATUS!='Аннулировано') order by ID_zak");
...
echo "</tbody></table>";
echo "<br><p><a style='cursor:pointer;' onclick='if(confirm(\"Подтвердить делегирование?\")){ vote2();};'><b style='font-size:200%'>Подтвердить делегирование.</b></a>";} echo "</div></tbody></table>";
...
var ids_zaks = '';
	...
	if ((nal_otr_2 == 0) && (nal_otr == 0)) {
		vote3(\"itrzadan_mne_dileg_func.php?p1=\"+ids_itrs+\"&p2=\"+newdp_itrs+\"&p3=\"+txt_itrs+\"&p8=\"+ids_zaks+\"&p5=".$nam_1['ID']."&p6=\"+idus2_itr+\"&p7=\"+idus3_itr+\"&p4=\"+komms_itrs);
	}else{
