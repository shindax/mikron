...
class Selector
{
	...
	var $page_menu_selected		= array('<b>', '</b>');	//шаблон для номера текущей страницы
	...
	*/
	...
	/*
	метод добавления массива возвращаемых полей
	$column_arr - массив полей
	*/
	function setColumnArray($column_arr)
	{
		foreach($column_arr as $row)
			$this->column_list[] = $row;
	}
	...
	*/
	function getResult($format=true, $field=null)
	{
		$sql = $this->getSql();
	...
	*/
	...
	/*
	МЕТОД УСТАРЕЛ И ОСТАВЛЕН ДЛЯ ВОЗМОЖНОГО ОТКАТА
	...
	$rewrite - флаг автоматического реврайта адреса. По умолчанию опущен
	*/
	function _getPageMenu($num_rows=null, $limit=null, $rewrite=false)
	{
		if(empty($this->page_menu) or empty($this->cache_metods_result))
		{
			...
			if($num_pages>1)
			{
				if($rewrite)
				{
					...
					$file = str_replace(UNIT_EXT, '', $file);
					$params = explode(VAR_SEPARATOR, $file);
					$url = implode('/', $segments).'/';
					$url_list = preg_split('/'.$index.$key_value_separator.'([[:alnum:]])*/', $file);
					if(!isset($url_list[1]))
					{
						$url_list[1] = null;
				...
				else
				{
					...
					if(isset($url_get[1]))
					{
						$url_list = preg_split('/'.$index.$key_value_separator.'([[:alnum:]])*/', $url_get[1]);
						if(!isset($url_list[1]))
						{
							$url_list[1] = null;
							$pref = $var_separator;
				...
				$this->page_url['first'] = $url_list[0].$pref.$index.$key_value_separator.'1'.$url_list[1];
				$this->page_url['final'] = $url_list[0].$pref.$index.$key_value_separator.$num_pages.$url_list[1];
				$this->page_url['all'] = $url_list[0].$pref.$index.$key_value_separator.'all'.$url_list[1];
				
				if(is_numeric($page))
				{
					if($page>1)
						$this->page_url['previous'] = $url_list[0].$pref.$index.$key_value_separator.($page-1).$url_list[1];
					if($page<$num_pages)
						$this->page_url['next'] = $url_list[0].$pref.$index.$key_value_separator.($page+1).$url_list[1];
					...
					{
						$url = $url_list[0].$pref.$index.$key_value_separator.($begin_i-1).$url_list[1];
						
						$page_menu .= '<a href="'.$url.'" '.$this->page_menu_a_options.'>...</a> ';
					...
					{
						$url = $url_list[0].$pref.$index.$key_value_separator.$i.$url_list[1];
							
						if($i!=$page)
							$page_menu .= '<a href="'.$url.'" '.$this->page_menu_a_options.'>'.$i.'</a>';
						else
							$page_menu .= $this->page_menu_selected[0].$i.$this->page_menu_selected[1];
					...
					{
						$url = $url_list[0].$pref.$index.$key_value_separator.($end_i+1).$url_list[1];
	...
	$rewrite - флаг автоматического реврайта адреса. По умолчанию опущен
	*/
	function getPageMenu($num_rows=null, $limit=null, $rewrite=false)
	{
		if(empty($this->page_menu) or empty($this->cache_metods_result))
		{
			if(!empty($num_rows))	
			...
			if($num_pages>1)
			{
				$this->page_url['first'] = $this->url(1, $rewrite);
				$this->page_url['final'] = $this->url($num_pages, $rewrite);
				$this->page_url['all'] = $this->url('all', $rewrite);
				
				if(is_numeric($page))
				{
					if($page>1)
						$this->page_url['previous'] = $this->url($page-1, $rewrite);
					if($page<$num_pages)
						$this->page_url['next'] = $this->url($page+1, $rewrite);
					...
					if($diapason_flag and $begin_i>1)
					{
						$url = $this->url($begin_i-1, $rewrite);
	...
	// Метод для работы с Bootstrsp
	function getPageMenuBootStrap($num_rows=null, $limit=null, $rewrite=false)
	{
		if(empty($this->page_menu) or empty($this->cache_metods_result))
		{
			if(!empty($num_rows))	
			...
			if($num_pages>1)
			{
				...
				if(is_numeric($page))
				{
					...
					if($diapason_flag and $begin_i>1)
					{
						...
						$page_menu .= '<li><a href="'.$url.'" '.$this->page_menu_a_options.'>...</a></li> ';
					...
					for($i=$begin_i; $i<=$end_i; $i++)
					{
						...
						if($i!=$page)
							$page_menu .= '<li><a href="'.$url.'" '.$this->page_menu_a_options.'>'.$i.'</a></li>';
						else
							$page_menu .= $this->page_menu_selected[0].$i.$this->page_menu_selected[1];
	...
	/*
	собирает URL для меню меню выбора страниц
	$page - номер страницы
	$rewrite - флаг автоматического реврайта адреса. По умолчанию опущен
	*/
	function url($page, $rewrite=false)
	{
		$params = $_GET;
		$params[$this->index] = $page;
