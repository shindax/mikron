...
class storage
{
	function __construct()
	{
		...
		if(isAjax())
		{
			if(isset($_POST['parent_specification']))
				$this->ajax_send_specification();
			else
				$this->ajax_send();
		}
	...
	function index_orders()
	{
		...
		$res->setWhere('EXISTS((SELECT * FROM okb_db_orders_ship WHERE order_id = ord.ID)) and (SELECT SUM(order_count) FROM okb_db_orders_ship WHERE order_id = ord.ID) < ord.DSE_COUNT or NOT EXISTS (SELECT * FROM okb_db_orders_ship WHERE order_id = ord.ID)');
		...
		if(!empty($res))
		{
			$html = '<h2>'.iconv('utf-8', 'cp1251', 'На складе').'</h2>
			...
			foreach($res as &$row)
			{
				if($row['summ'] > 0 and ($row['DSE_COUNT'] - $row['summ']) > 0 or $row['summ'] === null)
				{
					$html .= '<tr data-proj data-zak data-user-id="" data-id="'.$row['ID'].'" id="'.$row['ID'].'">
									...
									<td class="Field">
										<a href="#0" class="count">'.($row['DSE_COUNT'] - (int)$row['summ']).'</a>
									</td>
								</tr>';
				}
			}
			...
			print(iconv('utf-8', 'cp1251', '<h3>Нет активных позиций</h3>'));
	}
	...
	private function ajax_send_specification()
	{
		$json = new JSON(true);
		
		
		$parent = $this->db->select('okb_db_zakdet', 'ID', array('ID'=>(int)$_POST['parent_specification']));
		
		if(!$parent)
			exit;
		else
		{
			$id = $this->db->insert('okb_db_zakdet', array('PID'=>$_POST['parent_specification'], 'ID_zak'=>$_POST['id_zak']));
			
			$result = array();
			
			if($id)
			{
				$alt_array = array("Просмотр", "МТК", "Спецификация", "Расцеховка", "Удалить элемент", "Уверены, что хотите удалить элемент?", "Добавить вложенный элемент", "Добавить ссылку на элемент", "Отмена", "Копировать", "Элемент используется");
				
				foreach($alt_array as &$row)
					$row = iconv('utf-8', 'cp1251', $row);
				
				// dump($alt_array);
				
				$result['alt_array'] = $alt_array;
				$result['id'] = $id;
