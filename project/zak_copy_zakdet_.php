		...
		if ($to_zakdet = mysql_fetch_array($xxx)) 
		{
			...
			// Копируем МТК_OK, NAME, OBOZ основной сборки /////////////
				dbquery("Update ".$db_prefix."db_zakdet 
					Set 
					MTK_OK = '{$from_zakdet["MTK_OK"]}',
					NAME   = '{$from_zakdet["NAME"]}',
					OBOZ   = '{$from_zakdet["OBOZ"]}'
					WHERE (ID='".$to_id."')");
...
function CopyIzdOperitems($from_zakdet_ID,$to_zakdet_ID, $us_id) 
{
	...
	while($res = mysql_fetch_array($xxx)) 
	{
		...
		dbquery("	INSERT 
			...
			NORM_ZAK, 
			MORE) 
			VALUES ('".mktime(date("H"),date("i"),date("s"),date("m"),date("d"),date("Y"))."', '".$us_id."', '".$res4["ID_zak"]."', '".$to_zakdet_ID."', '".$res["ORD"]."', '".$res["ID_oper"]."', '".$res["ID_park"]."', '".$res["NORM"]."', '".$res["NORM_2"]."', '".$res["NORM_ZAK"]."', '".$res["MORE"]."')");
