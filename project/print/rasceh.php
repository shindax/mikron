...
if (isset($_GET["id"])) {
	...
	function OpenID($item,$n,$c,$cl) {
		...
		$xxx = dbquery("SELECT * FROM ".$db_prefix."db_operitems where (ID_zakdet = '".$item["ID"]."') order by ORD");
		$ceh = "";
		$cbr = "";
		while($res = mysql_fetch_array($xxx)) {
			$oper = dbquery("SELECT * FROM ".$db_prefix."db_oper where (ID = '".$res["ID_oper"]."')");
			$oper = mysql_fetch_array($oper);

			$stl = "border: 1px solid #fff; background: #fff;";
			$zadan = dbquery("SELECT * FROM ".$db_prefix."db_zadan where (ID_operitems = '".$res["ID"]."')");
			if ($zadan = mysql_fetch_array($zadan)) $stl = "border: 1px dashed #444; background: #f5f5f5;";
			if ($res["STATE"]=='1') $stl = "border: 1px solid #444; background: #ddd;";
			$ceh = $ceh.$cbr."<span style='".$stl."'>".$oper["CODE"]."</span>";
			$cbr = "-";
		}
	   ...
	   // 
		...
		if (!$isclds) {
			...
			echo "<td style='background: #$cl;'><i>".FVal($item,"db_zakdet","RCOUNT").$mark."</i></td>";
		}
		...
		if ($isclds) {
			...
			echo "<td style='background: #$cl;'><i><b>".FVal($item,"db_zakdet","RCOUNT").$mark."</b></i></td>";
		}

		echo "<td style='background: #fff; padding: 2px;'>".$ceh."</td>";
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	echo "<tr>";
		echo "<td colspan='".(5+$max_n)."' style='border-bottom: 2px solid black; text-align: center;'><b style='font-size: 18pt;'>ЗАКАЗ № ".$name."</b></td>";
	echo "</tr>";
	echo "<tr>";
		echo "<td rowspan='2' colspan='".$max_n."' style='border: 2px solid black; vertical-align: middle; text-align: center;'>№</td>\n";
		echo "<td rowspan='2' style='border: 2px solid black; vertical-align: middle; text-align: center;'>Наименование ДСЕ</td>\n";
		echo "<td rowspan='2' width='180' style='border: 2px solid black; vertical-align: middle; text-align: center;'>№ чертежа ДСЕ</td>\n";
		echo "<td colspan='2' style='border: 2px solid black; vertical-align: middle; text-align: center;'>Кол-во<br>ДСЕ</td>\n";
		echo "<td rowspan='2' width='240' style='border: 2px solid black; vertical-align: middle; text-align: center;'>Расцеховка</td>\n";
	echo "</tr>";
	echo "<tr>";
	...
	$SCount2 = $SCount*$res["COUNT"];

	echo "<tr>";
		echo "<td colspan='".(2+$max_n)."' style='border-top: 2px solid black; text-align: right;'><b>ИТОГО деталей в сборке:</b></td>";
		echo "<td style='border-top: 2px solid black; text-align: left;'>".$SCount."</td>";
		echo "<td style='border-top: 2px solid black; text-align: left;'>".$SCount2."</td>";
