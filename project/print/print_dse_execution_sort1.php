  ...
  function PrintSortedByObozDSETable( $arr, $pdate1, $pdate2, $ops )
{
    ...
    foreach( $arr AS $item )
    {
        ...
        if( ( ( $prev_oboz != $zakdet_oboz ) || ! $first_pass ++ ) && ! $total )
        {
            $head = ( $line_num ++ ).". ".$zakdet_name." ".$zakdet_oboz;
            $str .= "<tr class='first'><td style='background-color:#DDD' class='Field AC' colspan='8'><b>".$head."<b></td></tr>";
        ...
        $str .= "<tr class='".$class."'>";
        $str .= "<td ".$s." class='Field AC'><b>".$zak_name."</b></td>";
 ...
 function sortByNameObozZak( $a, $b ) 
{
    if ( $a['zakdet_name'] == $b['zakdet_name'] ) 
    {
        ...
        {
            if ( $a['zak_name'] == $b['zak_name'] )
                return 0;
            
            return ($a['zak_name'] < $b['zak_name']) ? -1 : 1 ;
        }
