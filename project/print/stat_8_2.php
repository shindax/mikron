...
if ($step==1) {
	...
	echo "<h2>Состояние выполнения заказов - выбор заказов (+Материал)</h2><br/><a style='float:right' href='/?do=show&formid=230'>Аннулированные</a><br/><br/>";
...
if ($step==2) {
   ...
   // Шапка
	...
	echo "<h2>Состояние выполнения заказов</h2><input type=button value='Экспортировать в Excel' onclick='window.open(\"project/print/stat_8_2_2_2.php?p2=".$arr_zak_ids."\");'><br><br>";
...
LEFT JOIN okb_db_sort ON okb_db_sort.ID = okb_db_zn_zag.ID_sort
			...
			echo "<td name='replac2'  class='Field'>" . $mat['MaterialName'] . "<br/>" . $mat['MaterialSort'] ."</td>\n";
			echo "</tr>\n";
