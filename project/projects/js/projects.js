...
// Восстанавливаем изначальный порядок строк
        if( rows_arr[ id ] )
     {
       ...
       $( img ).attr( {'src':'uses/collapse.png', 'data-state':'0', 'title': 'Открыть'});
       display = 'none';
       arrow = '&#9658;'
    ...
    {
        $( img ).attr( {'src':'uses/expand.png', 'data-state': '1', 'title': 'Закрыть'});
        display = 'table-row' ;
        arrow = '&#9660;' ;
...
function navBarClick()
{
  ...
  // Отправляем запрос
    ...
    $.post(
      "project/projects/ajax.getData.php",
        {
          id : id,
          user_id : user_id
        },
                    function( data )
                    {
                        ...
                        {
                          id = 'all';
                          $('#user_chart_select').hide();
                          $('#user_chart_select_img').hide();
                          $('.chart_div div:last-child').css('border-left','none');
                        }
                        else
                        {
                          $.post(
                            "project/projects/ajax.getUserSelect.php",
                              {
                                id : id
                              },
                                          function( select_data )
                                          {
                                            $('.chart_div div:last-child').css('border-left','1px dashed  black');
                                              $('#user_chart_select').empty().html( select_data ).show();

                                              user_chart_select_change();

                                              var user_id = $('#user_chart_select').find('option:first-child').val();
                                              var src = '/uses/working_calendar_img/work_cal_' + user_id + '.png';
                                              $('#user_chart_select_img').attr('src', src).show();
                                          }
                          );
                        }

                        var src = '/uses/working_calendar_img/project_hours_' + id + '.png';
                        $( '#project_table' ).empty().append( data );
                        adjust_all();
                        $( el ).find('a').removeClass('odd');
                        $('ul.pagination li a:odd').addClass('odd');
                        $( 'ul.pagination li').removeClass('active');
                        $( el ).addClass('active').find('a').removeClass('odd');
                        stopLoadingAnimation();
                        $('img.chart').attr( { 'src' : src + '?1234' });
                    }
                );
...
function adjust_all()
{
    $('li.page-item').unbind('click').bind('click', navBarClick );
    $('ul.pagination li a:odd').addClass('odd');
...
// ************************************************************************************************
 ...
 // Удаляем лишние combobox от jquery-ui ???
  ...
  var rec_count = $('tr.project').length;

  $('#rec_count').text('\u041A\u043E\u043B\u0438\u0447\u0435\u0441\u0442\u0432\u043E \u043F\u0440\u043E\u0435\u043A\u0442\u043E\u0432 : ' + rec_count )

//  alert( rec_count );

  $('#user_chart_select').unbind('change').bind('change', user_chart_select_change );
};

function user_chart_select_change()
{
  var user_id = $( "#user_chart_select option:selected" ).val();

    $.post(
      "project/projects/ajax.MakeUserChart.php",
        {
          user_id : user_id
        },
                    function( image_name )
                    {
//                      alert( image_name );
                        var src = '/uses/working_calendar_img/'+ image_name + '.png';
                        $('#user_chart_select_img').attr('src', src).show();
                    }
          );
