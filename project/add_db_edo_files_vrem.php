<?php
  ...
  ('".$last_4['KOMM']."', '".$ltas_3[0]."', '".$last_4['OTVET_INOUT']."', '0', '".$last_4['ID_clients']."', '".$last_4['ID_clients_contacts']."', '".$last_4['NAME_OUT']."', 
  ...
  $resxx8 = dbquery("SELECT ID_resurs FROM ".$db_prefix."db_shtat where (ID_special='17') ");
  $last_6 = mysql_fetch_row($resxx8);
  $resxx9 = dbquery("SELECT ID_resurs FROM ".$db_prefix."db_contacts where (ID='".$last_4['ID_contacts']."') ");
  $last_7 = mysql_fetch_row($resxx9);
  $resxx10 = dbquery("SELECT ID FROM ".$db_prefix."db_resurs where (ID_users='".$user['ID']."') ");
  $last_8 = mysql_fetch_row($resxx10);
  
  dbquery("INSERT INTO ".$db_prefix."db_edo_vremitr (ID_contacts) VALUES ('".$cur_new_ins_id."')");
  dbquery("INSERT INTO ".$db_prefix."db_itrzadan (ID_zak, DATE_PLAN, TIME_PLAN, STARTDATE, STARTTIME, CDATE, CTIME, ID_users, ID_users2, ID_users3, TXT, EUSER, ID_edo, TIP_JOB, STATUS, TIP_FAIL, ETIME) 
  VALUES ('".$last_4['ID_zak']."', '".date("Ymd", mktime(0, 0, 0, date("m"), date("d"), date("Y"))+345600)."', '17:00:00', '".date("Ymd")."', '08:00:00', '".date("Ymd")."', '".date("H:i:s")."', '".$last_7[0]."', 
  '".$last_6[0]."', '".$last_7[0]."', 'Подготовить ответ на входящий документ.', '".$last_8[0]."', '".$cur_new_ins_id."', '1', 'Новое', '0', '".mktime(date("H"), date("i"), date("s"), date("m"), date("d"), date("Y"))."')");
  $cur_new_ins_id_itr = mysql_insert_id();
  dbquery("INSERT INTO ".$db_prefix."db_itrzadan_statuses (DATA, STATUS, USER, ID_edo, TIME) VALUES ('".date("Ymd")."', 'Новое', '".$last_7[0]."', '".$cur_new_ins_id_itr."', '".mktime(date("H"), date("i"), date("s"), date("m"), date("d"), date("Y"))."')");
...
location.href='index.php?do=show&formid=110&id=".$cur_new_ins_id."';
</script>";
?>