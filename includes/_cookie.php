...
/////////////////////////////////////////////////////////////////////////////////////
	...
	function GetUserRights() {
		...
		while ($group = mysql_fetch_array($result)) {
			if (in_array($group["ID"],$ids)) $res = $res."\n".$group["RIGHTS"];
		}
		$user_rights = explode("\n",$res);
	}
	...
	function GetShowRights() {
		...
		while ($group = mysql_fetch_array($result)) {
			if (in_array($group["ID"],$ids)) $res = $res."|".$group["RIGHTS"];
		}
...
/////////////////////////////////////////////////////////////////////////////////////
	...
	function GetUserWay() {
		...
		if ($url!==$urls[0]) {
			if (isset($_GET["back"])) {
				if (count($urls)>1) {
					for ($j=1;$j < count($urls);$j++) $newurls[] = $urls[$j];
				...
				if (count($urls)<10) {
					$newurls[] = $url;
					for ($j=0;$j < count($urls);$j++) $newurls[] = $urls[$j];
				} else {
					$newurls[] = $url;
	...
	if (isset($_COOKIE["user_pass"])) $userpass = $_COOKIE["user_pass"];
	if (isset($_POST["user_login"])) $userpass = trim($_POST["user_login"])."=".md5(trim($_POST["user_pass"]));
	if (isset($_GET["unlogin"])) $userpass = "0";
...
/////////////////////////////////////////////////////////////////////////////////////
	...
	if (isset($_GET['addopened'])) {
		$opened=$opened.$_GET['addopened'];
	}
	if (isset($_GET['open'])) {
		$opened=str_replace("|".$_GET['open']."|","",$opened);
		$opened=$opened."|".$_GET['open']."|";
	}
